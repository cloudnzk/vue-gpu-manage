<template>
  <div class="leave-main">
    ...
    <div class="leave-bottom">
      <div class="leave-bottom-top"></div>
      ...
      <!-- 审核弹窗 -->
      <el-dialog title="审核" v-model="detailShow" width="40%">
        <el-form
          ref="ruleForm"
          label-width="168px"
          label-suffix=":"
        ...
        <template #footer>
          <span class="dialog-footer">
            <el-button type="success" @click="auditHandler('pass')"
              >通过</el-button
            >
            <el-button type="primary" @click="auditHandler('refuse')"
              >驳回</el-button
            >
          </span>
        </template>
  ...
  proxy.$refs.ruleForm.validate(async (valid) => {
    if (valid) {
      try {
        ...
        proxy.$store.commit("saveNoticeCount", store.state.noticeCount - 1);
      } catch (error) {
