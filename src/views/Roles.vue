<template>
  <div class="roles-main">
    ...
    <div class="roles-bottom">
      ...
      <!-- 新增弹窗 -->
      <el-dialog title="新增角色" v-model="addShow" width="35%">
        <el-form
          :model="addForm"
          :rules="rules"
          ref="addForm"
      ...
      <!-- 设置权限弹窗 -->
      <el-dialog title="设置权限" v-model="permissionShow" width="35%">
        <el-form
        ...
        <template #footer>
          <span class="dialog-footer">
            ...
            <el-button type="primary" @click="perssionSubmitHandler"
              >确 定</el-button
            >
          </span>
        </template>
...
export default {
  ...
  methods: {
    ...
    dialogSubmitHandler() {
      this.$refs.addForm.validate(async (vaild) => {
        if (vaild) {
          ...
          await this.$api.postRolesC_U_D(params);
    ...
    handlerSetPermission(row) {
      ...
      this.$nextTick(() => {
        // 将角色的权限列表查出来，赋值给表单，设置选中的节点
        // permissionList 是 roles 表的一个属性，包含了菜单和按钮的权限列表。这里只赋值按钮的权限
    ...
    async perssionSubmitHandler() {
      ...
      // 为什么不把菜单设置为全选，考虑到以后可能会新增一些按钮，如果全选了会自动分配权限
      const halfCheckedKeys = this.$refs.menuTree.getHalfCheckedKeys(); // 获取真正半选中状态的节点key
      const checkedKeys = []; // 选中的最后一级菜单，也就是按钮
      // 半选中状态节点-这里的半选中状态其实是虚拟的，人为把他当做半选中，
      // 因为elementUI的树控件只要最后一个层级节点都被选中了，那么父节点自动就全选状态了
